...
class BlumTod:
    def __init__(self, id, query, proxies, config: Config):
        ...
        if user is None:
            self.valid = False
            self.log(f"{red}Os dados desta conta estão no formato incorreto.")
            return None
    ...
    def log(self, msg):
        ...
        print(
            f"{black}[{now}]{white}-{blue}[{white}Conta {self.p + 1}{blue}]{white} {msg}{reset}"
        )
    ...
    async def http(self, url, headers, data=None):
        while True:
            try:
                ...
                self.log(f"{yellow}Conexão encerrada sem resposta !")
                await asyncio.sleep(3)
                continue
    ...
    async def login(self):
        ...
        if not token:
            ...
            if "signature is invalid" in message:
                self.log(f"{red}Os dados estão no formato incorreto ou desatualizados.")
                return False
        ...
        self.log(f"{green}Token de acesso obtido com sucesso !")
    ...
    async def start(self):
        ...
        self.log(f"{green}Conectado a {white}{first_name}")
        ...
        if res.status_code == 404:
            self.log(f"{yellow}Já fez check-in hoje !")
        else:
            res = await self.http(checkin_url, self.headers, "")
            self.log(f"{green}Check-in de hoje realizado com sucesso !")
        while True:
        ...
        self.log(f"{green}referral saldo : {white}{amount_claim}")
        if can_claim:
            ...
            if clres.json().get("claimBalance") is not None:
                ...
                self.log(f"{red}Falha ao reivindicar recompensa de indicação !")
        if self.cfg.auto_claim:
            while True:
                if farming is None:
                    _res = await self.http(farming_start_url, self.headers, "")
                    if _res.status_code != 200:
                        self.log(f"{red}Falha ao iniciar a coleta !")
                    else:
                        self.log(f"{green}Coleta iniciada com sucesso !")
                        farming = _res.json()
                if farming is None:
                    res = await self.http(balance_url, self.headers)
                    farming = res.json().get("farming")
                ...
                if timestamp > (end_farming / 1000):
                    ...
                    if res_.status_code != 200:
                        self.log(f"{red}Falha ao reivindicar a coleta !")
                    else:
                        self.log(f"{green}Coleta reivindicada com sucesso !")
                        farming = None
                        continue
                else:
                    self.log(f"{yellow}Ainda não é hora de reivindicar a coleta !")
                end_iso = (
            ...
            if res:
                for tasks in res.json():
                    if isinstance(tasks, str):
                        self.log(f"{yellow}Falha ao obter lista de tarefas !")
                        break
        ...
        if self.cfg.auto_game:
            ...
            try:
                ...
                if "payload" in data:
                    ...
                    self.log(f"{red}Falha ao iniciar jogos - {e}")
                    self.log(f"{red}Jogos não estão disponíveis no momento!!")
                    game = False
    ...
    async def solve(self, task: dict):
        ...
        while True:
            if task_status == "FINISHED":
                self.log(f"{yellow}já completou o ID da tarefa {white}{task_title} !")
                return
            if task_status == "READY_FOR_CLAIM" or task_status == "STARTED":
                ...
                if _status == "FINISHED":
                    self.log(f"{green}Tarefa concluída com sucesso id {white}{task_title} !")
                    return
...
{green}total proxy :{white} {len(proxies)}
    ...
    {green}1{white}.{green}) {white}ativar/desativar reivindicação automática ({(green + "active" if config.auto_claim else red + "non-active")})
    {green}2{white}.{green}) {white}ativar/desativar tarefa de resolução automática ({(green + "active" if config.auto_task else red + "non-active")})
    {green}3{white}.{green}) {white}ativar/desativar o jogo automático ({(green + "active" if config.auto_game else red + "non-active")})
    ...
    {green}7{white}.{green}) {white}iniciar bot (modo de sincronização)
        ...
        #{green}3{white}.{green}) {white}set on/off auto play game ({(red + "Jogos não estão disponíveis no momento!!")})
        opt = None
        if args.action:
            opt = args.action
        else:
